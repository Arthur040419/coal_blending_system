 ...
 *       质量优先、价格次要。</li>
 *   <li><b>Step 2 经验配比规则</b>：仅在综合分排序前 4 个煤种中生成多组人工经验候选——
 *       N=2（主煤 60% + 辅煤 40%，对应"主辅 6:4"经验）与
 *       N=3（主煤 50% + 副煤 30% + 辅煤 20%，对应"主副辅 5:3:2"经验）；
 *       不进行小步长试错调整，不修改经验配比模板。
 *       候选物料不足 3 种时，仅生成 N=2 候选。</li>
 *   <li><b>Step 3 统一决策校验</b>：将每组候选的配比代入线性加权计算配合煤指标，
 *       并复用系统评分、库存校验、风险识别与 Pareto 标记口径。</li>
 *   <li><b>Step 4 择优 + 显式缺失</b>：按"可执行优先 → Pareto 等级更优 →
 *       系统综合评分更高 → 吨煤成本更低"的优先级择优，<b>仅输出一个最终方案</b>。
 *       算法不查规则知识库、不查历史案例、不查 RAG，
